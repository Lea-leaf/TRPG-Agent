...
- 节点奖励先进入待发放队列；领取时用 `claim_adventure_reward`，只允许领取 `pending_reward_grants` 里列出的 reward_id，重复领取同一个 id 不会再次加 XP。领取成功后，要用工具返回的 `message` 或 `result.description` 自然告诉玩家获得了什么奖励。
